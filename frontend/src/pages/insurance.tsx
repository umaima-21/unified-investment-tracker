...
import { Plus, Upload, Shield, Calendar, User, FileText, Trash2, RefreshCw } from 'lucide-react'
...
export function InsurancePage() {
  ...
  const clearAllMutation = useMutation({
    mutationFn: async () => {
      const { data } = await api.delete(endpoints.insurance.clearAll)
      return data
    },
    onSuccess: (data) => {
      ...
      toast({
        title: 'Success',
        description: data.message || `Cleared ${data.deleted_count || 0} insurance policy/policies`,
      })
    },
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.response?.data?.detail || 'Failed to clear insurance policies',
        variant: 'destructive',
      })
    },
  })
  ...
  const importJsonMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        title: 'Success',
        description: data.message || `Imported ${data.imported_count || 0} insurance policy/policies successfully`,
      })
    },
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.response?.data?.detail || 'Failed to import insurance policies from JSON',
        variant: 'destructive',
      })
    },
  })
  ...
  const handleLoad = () => {
    importJsonMutation.mutate('data/insurance.json')
  }
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage your insurance policies (payouts - not included in portfolio value)
          </p>
        </div>
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="py-10 text-center">
            <p className="text-destructive font-medium mb-2">Error loading insurance policies</p>
        ...
        <div className="flex gap-2">
          {holdings && Array.isArray(holdings) && holdings.length > 0 && (
            <Button
              variant="outline"
              onClick={() => {
                if (confirm('Are you sure you want to delete all insurance policies? This action cannot be undone.')) {
          ...
          <Button
            ...
            onClick={handleLoad}
            disabled={clearAllMutation.isPending || importJsonMutation.isPending}
            title="Import data from data/insurance.json file"
          >
        ...
        <Card>
          <CardContent className="py-10 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Click "Import from JSON" to load your insurance policy data
            </p>
          </CardContent>
        </Card>
      ) : (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  ...
                  {policy.duration_years && (
                    <div className="space-y-1">
                      ...
                      <div className="font-medium text-sm">{policy.duration_years} years</div>
                    </div>
                  )}
                  {policy.nominee && (
                    <div className="space-y-1">
                ...
                <div className="mt-4 p-4 bg-amber-50 dark:bg-amber-950/20 rounded-lg border border-amber-200 dark:border-amber-900">
                  ...
                  <div className="mt-3 pt-3 border-t border-amber-200 dark:border-amber-800 text-center">
                    <p className="text-xs text-muted-foreground">
                      Note: Insurance policies are payouts and are not included in portfolio Current Value calculation
                    </p>
                  </div>
                </div>
